...
export default function InspectAttemptModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-neutral-900/80 backdrop-blur-sm">
      <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        ...
        <div className="space-y-3 mb-5">
          <div className="flex justify-between items-center bg-neutral-900 rounded-xl px-4 py-3">
            ...
            <span
              className={`font-mono font-bold text-sm ${(inspectEntry.tabSwitchCount ?? 0) > 2 ? "text-red-400" : "text-neutral-200"}`}
            >
              {inspectEntry.tabSwitchCount ?? 0}
            </span>
          </div>
          <div className="flex justify-between items-center bg-neutral-900 rounded-xl px-4 py-3">
            <span className="text-sm text-neutral-400">Time off tab</span>
            <span
              className={`font-mono font-bold text-sm ${(inspectEntry.timeOffTab ?? 0) > 10 ? "text-red-400" : "text-neutral-200"}`}
            >
        ...
        <div className="space-y-3 mb-5 border-t border-neutral-800 pt-4">
          <div className="flex items-center justify-between">
            <span className="text-sm text-neutral-400">Elo deduction</span>
            <input
              type="number"
              min={0}
              value={eloPenalty}
              ...
              className="w-24 bg-neutral-900 border border-neutral-700 rounded-lg px-3 py-1.5 text-sm text-white text-right font-mono focus:outline-none focus:border-neutral-500"
            />
          </div>
          <label className="flex items-center gap-3 cursor-pointer select-none">
            ...
            />
            <span className="text-sm text-neutral-300">Erase attempt</span>
          </label>
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 py-2.5 rounded-xl text-neutral-400 hover:text-white border border-neutral-800 hover:border-neutral-600 transition-colors text-sm font-semibold"
          >
            Close
          </button>
          <button
