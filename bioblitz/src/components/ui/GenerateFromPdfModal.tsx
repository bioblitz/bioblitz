...
export default function GenerateFromPdfModal({ onApply, onClose }: GenerateFromPdfModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-neutral-950 border border-neutral-800 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          ...
          {/* PDF upload zone */}
          <div>
            <label className="block text-xs font-medium text-neutral-400 mb-2">PDF File</label>
            {pdfFile ? (
              <div className="flex items-center gap-3 px-4 py-3 bg-neutral-900 border border-neutral-700 rounded-xl">
                ...
                <span className="text-sm text-neutral-200 truncate flex-1">{pdfFile.name}</span>
                <button
              ...
              <div
                ...
                className={`cursor-pointer flex flex-col items-center justify-center gap-2 p-8 rounded-xl border-2 border-dashed transition-colors ${
                  dragging ? "border-neutral-500 bg-neutral-800/60" : "border-neutral-700 bg-neutral-900/40 hover:border-neutral-600 hover:bg-neutral-800/40"
                }`}
              >
                <Upload className="w-6 h-6 text-neutral-500" />
                <span className="text-sm text-neutral-400">Drop a PDF here, or click to browse</span>
                <input
                  ref={fileInputRef}
                  type="file"
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-neutral-900 border border-neutral-700 rounded-xl px-4 py-3 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:border-neutral-500 resize-none"
            />
          </div>
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                {generated.map((q, i) => (
                  <div key={i} className="bg-neutral-900 border border-neutral-800 rounded-xl overflow-hidden">
                    ...
                    >
                      <span className="text-xs text-neutral-500 shrink-0">Q{i + 1}</span>
                    ...
                    {expandedIdx === i && (
                      <div className="px-4 pb-3 space-y-1.5 border-t border-neutral-800/60 pt-3">
                        {q.choices.map((c, ci) => (
                          <div
                            key={ci}
                          ...
                          >
                            <span className="text-xs uppercase text-neutral-500 w-4 shrink-0">
                        ...
                        {q.solution && (
                          <p className="text-xs text-neutral-500 pt-1 italic">{q.solution}</p>
                        )}
                      </div>
                    )}
                  </div>
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-neutral-800 flex items-center justify-between gap-3 shrink-0">
          <button
            type="button"
          ...
          {generated && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-4 py-2.5 text-sm font-bold text-neutral-300 hover:text-white border border-neutral-700 hover:border-neutral-500 rounded-xl transition-colors"
              >
                Append
              </button>
              <button
                ...
                className="px-4 py-2.5 text-sm font-bold bg-neutral-700 hover:bg-neutral-600 text-white rounded-xl transition-colors"
              >
                Replace all
              </button>
            </div>
