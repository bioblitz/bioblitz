...
export default function ProfileFriendsPanel({
  ...
  return (
    <div className="bg-neutral-950/50 backdrop-blur-sm border border-neutral-800 rounded-3xl p-6 flex flex-col gap-4 shadow-xl h-[42rem]">
          ...
          <div className="flex flex-col gap-2">
            {friends.map((friend) => (
              <Link
                ...
                className="flex items-center gap-3 p-2 rounded-xl hover:bg-neutral-800 transition"
              >
                {friend.photoURL ? (
                  <img
                    src={friend.photoURL}
      ...
      {isOwnProfile && (
        <div className="bg-neutral-950/50 backdrop-blur-sm rounded-3xl p-0 flex flex-col gap-4 shadow-xl">
          ...
          {loadingFriends ? (
            ...
            <p className="text-neutral-400 text-sm">No incoming requests.</p>
          ) : (
            <div className="flex flex-col gap-2 max-h-32 overflow-y-auto pr-2 [scrollbar-width:thin] [scrollbar-color:#8b5cf6_transparent] [&::-webkit-scrollbar]:w-3 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-neutral-500/30 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-neutral-500  [&::-webkit-scrollbar-button]:h-0 [&::-webkit-scrollbar-button]:w-0 [&::-webkit-scrollbar-corner]:bg-transparent">
              {incomingRequests.map((request) => (
                <div
                  key={request.uid}
                  className="flex items-center justify-between p-2 rounded-xl hover:bg-neutral-800 transition"
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="px-3 py-1 bg-neutral-800 rounded-full text-neutral-400 text-sm hover:text-white"
                    >
                      Decline
                    </button>
                  </div>
          ...
          <div className="mt-4 pt-4 border-t border-neutral-800 flex flex-col gap-3">
            <div className="flex gap-2 items-center">
              <div className="relative shrink-0 group">
                ...
                <span className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-max px-2 py-1 text-xs text-white bg-neutral-900 rounded-md border border-neutral-800 opacity-0 pointer-events-none transition-opacity duration-200 group-hover:opacity-100">
