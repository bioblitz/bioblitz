...
import { 
    getFirestore, collection, getDocs, doc, getDoc, updateDoc, setDoc, serverTimestamp, arrayUnion 
} from "firebase/firestore";
import { app } from "@/lib/firebase";
import { DailyPuzzle } from "@/lib/potd"; // Import shared interface
...
export default function PotdClient({
  ...
  // DATA STATE (Initialized directly from Server Props)
  const [puzzles] = useState<DailyPuzzle[]>(initialPuzzles);
  
  // LOADING STATE (Only for User Data now!)
  ...
  const fetchUserData = async (uid: string) => {
    try {
      ...
      if (userSnap.exists()) {
          ...
          // Optimized Read: Get Completed Games from Array
  ...
  // NOTE: The previous `fetchPuzzles` useEffect is DELETED. 
  // We now use the `initialPuzzles` prop passed from the Server Cache.
  ...
  const handleSubmit = async (puzzle: DailyPuzzle) => {
    ...
    try {
        ...
        // 2. Streak & Array Update
        const userDoc = await getDoc(userRef);
        const userData = userDoc.data();
        
        const now = new Date();
        const currentPstDate = new Date(now.toLocaleString("en-US", { timeZone: "America/Los_Angeles" }));
        ...
        if (!lastStreakDate) {
            ...
            const lastPstDate = new Date(lastStreakDate.toLocaleString("en-US", { timeZone: "America/Los_Angeles" }));
  ...
  const isTodayCompleted = todaysPuzzle ? playedGameIds.has(todaysPuzzle.id) : false;
  const activeFilterCount = (statusFilter !== "All" ? 1 : 0) + (topic !== "All Topics" ? 1 : 0);

  const filteredArchive = archivePuzzles.filter((puzzle) => {
    const matchesSearch = puzzle.title.toLowerCase().includes(searchQuery.toLowerCase()) || puzzle.questionText.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        {/* LOADING: Only waiting for user data now (Puzzles are instant) */}
        {userDataLoading ? (
            <div className="flex flex-col justify-center items-center py-32 space-y-4">
                <Loader2 className="w-10 h-10 text-orange-500 animate-spin" />
                <p className="text-zinc-500 text-sm font-medium animate-pulse">Loading Your Progress...</p>
            </div>
        ) : (
                ...
                <section className="relative">
                    {todaysPuzzle ? (
                        ...
                        (isTodayCompleted && !viewAnyway) ? (
                             <div className="relative overflow-hidden rounded-3xl bg-zinc-900 border border-zinc-800 shadow-2xl p-12 text-center animate-in fade-in duration-500">
                                ...
                                <div className="relative z-10 flex flex-col items-center">
                                    ...
                                    <p className="text-zinc-400 text-lg max-w-xl mx-auto mb-8">
                                        Great work! You've kept your streak alive today. Come back tomorrow for a new challenge.
                                    </p>
                                    
                                    <div className="flex flex-col sm:flex-row gap-4">
                                        <button 
                                            onClick={() => setShowArchive(true)}
                                            className="bg-white text-black hover:bg-zinc-200 px-8 py-3 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2"
                                        >
                                        ...
                                        <button 
                                            ...
                                            className="bg-zinc-800 text-zinc-300 hover:bg-zinc-700 hover:text-white px-6 py-3 rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-2 border border-zinc-700"
                                        >
                             ...
                             <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-zinc-900 to-zinc-950 border border-zinc-800 shadow-2xl group text-center">
                                ...
                                <div className="relative z-10 p-8 md:p-10 flex flex-col items-center">
                                    <div className="flex flex-wrap items-center justify-center gap-4 mb-6">
                                        <span className={`px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider ${getTopicColors(todaysPuzzle.topic).bg}`}>
                                            {todaysPuzzle.topic}
                                        </span>
                                        <span className="flex items-center gap-1.5 text-xs font-medium text-zinc-500 border border-zinc-800 px-2 py-1 rounded-md">
                                            {todaysPuzzle.multiSelect ? <ListChecks className="w-3 h-3" /> : <MousePointerClick className="w-3 h-3" />}
                                            {todaysPuzzle.multiSelect ? "Multi-Select" : "Single Choice"}
                                        </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-3 w-full max-w-2xl mx-auto">
                                        {todaysPuzzle.options.map((option) => {
                                            ...
                                            const isCorrectKey = todaysPuzzle.correctAnswer.includes(option.key);
                                            
                                            const showResults = isSubmitted || (viewAnyway && isTodayCompleted);
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    onClick={() => handleOptionClick(option.key, todaysPuzzle.multiSelect)}
                                                    className={`
                                                ...
                                                <div className={`p-6 rounded-2xl border mb-6 flex flex-col items-center gap-3 ${isCorrect ? "bg-green-500/10 border-green-500/20" : "bg-red-500/10 border-red-500/20"}`}>
                                                    {isCorrect ? (
                                                        <>
                                                            ...
                                                            <h3 className="text-xl font-bold text-green-400">Correct! Great Job!</h3>
                                                            {streakUpdated && <p className="text-sm text-green-300 font-medium">🔥 Streak Incremented!</p>}
                                                        </>
                                                    ) : (
                                                        <>
                                                            ...
                                                            <h3 className="text-xl font-bold text-red-400">Incorrect. Keep Learning!</h3>
                                                        </>
                                                    )}
                                                </div>
                                            )}
                        ...
                        <div className="p-12 rounded-3xl bg-zinc-900 border border-zinc-800 text-center">
                            ...
                            <h3 className="text-2xl font-bold text-white mb-2">No Problem for Today (Yet)</h3>
                            <p className="text-zinc-400">Check back later or browse the archive below!</p>
                        </div>
                    )}
                </section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {filteredArchive.length === 0 ? (
                                ...
                                filteredArchive.map((puzzle) => {
                                    ...
                                    return (
                                        <Link key={puzzle.id} href={`/potd/${puzzle.id}`} className="block group">
                                            <div className={`relative h-full flex flex-col justify-between bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${theme.shadow}`}>
                                                <div className="p-5">
                                                    ...
                                                    <h2 className="text-lg font-bold text-white mb-2 line-clamp-2">{puzzle.title}</h2>
                                                    <p className="text-sm text-zinc-500 line-clamp-2 mb-3">{puzzle.questionText}</p>
                                                    <div className="flex items-center text-sm text-zinc-400 mt-auto">
                                                        <Calendar className="w-3 h-3 mr-2" />
                                                        <span className="truncate">{new Date(puzzle.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </Link>
