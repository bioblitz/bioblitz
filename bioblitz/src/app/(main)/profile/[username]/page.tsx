...
export default function ProfilePage() {
  ...
  return (
    <main
      className={`${inter.className} min-h-screen bg-neutral-900 text-neutral-100 pt-24 pb-12 pl-16 pr-4 sm:pr-6 lg:pr-8 relative overflow-hidden`}
    >
              ...
              loadingElo ? (
                <div className="space-y-3 animate-pulse">
                  ...
                  <div className="flex gap-2 justify-end">
                    {[...Array(5)].map((_, i) => (
                      <div key={i} className="h-3 w-10 bg-neutral-800 rounded-full" />
                    ))}
                  </div>
                </div>
              ) : (
                ...
                <div className="w-1/2 flex flex-col gap-3">
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1 [scrollbar-width:thin] [scrollbar-color:#52525b_transparent]">
                    {loadingFriends ? (
                      ...
                      <p className="text-neutral-500 text-sm py-4">No friends yet.</p>
                    ) : (
                      friends.map((friend) => (
                        <Link
                          key={friend.uid}
                  ...
                  {authUser?.uid === profileUid && (
                    <>
                      <div className="flex flex-col gap-2">
                        <p className="text-xs font-semibold text-neutral-400">Add friend</p>
                        <div className="relative">
                          ...
                          {friendUsernameInput.trim() && !isSearching && userSearchResults.length === 0 && (
                            <div className="absolute top-full left-0 right-0 mt-1 z-20 bg-neutral-900 border border-neutral-800 rounded-xl px-3 py-2.5 shadow-lg">
                              <p className="text-neutral-500 text-xs">No users found</p>
                            </div>
                          )}
                        ...
                        {addFriendSuccess && (
                          <p className="text-xs text-neutral-400">{addFriendSuccess}</p>
                        )}
                      </div>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="space-y-1.5 overflow-y-auto max-h-48 [scrollbar-width:thin] [scrollbar-color:#52525b_transparent]">
                            {incomingRequests.map((req) => (
                              <div key={req.uid} className="flex items-center justify-between gap-2 p-2 rounded-xl hover:bg-neutral-800/50">
 ...
 onError={(e) => { (e.currentTarget as HTMLImageElement).src = "/images/logo.svg"; }} />
                  ...
                  {authUser?.uid !== profileUid && (
                    <p className="text-neutral-500 text-sm pt-4">Only visible to the profile owner.</p>
                  )}
                </div>
              </div>
            )}
              ...
              loadingSets ? (
                <div className="space-y-4 animate-pulse">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[...Array(30)].map((_, i) => (
                      <div key={i} className="w-8 h-8 rounded-lg bg-neutral-800" />
                    ))}
                  </div>
                </div>
              ) : (
        ...
        <Link
          ...
          className="group block rounded-3xl border border-neutral-800 bg-neutral-950/70 overflow-hidden shadow-xl"
        >
          <div
            className="relative h-40 sm:h-48 bg-neutral-900/60 flex items-center justify-center overflow-hidden"
            style={
