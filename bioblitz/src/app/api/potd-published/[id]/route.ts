...
export async function PATCH(
  ...
  const docSnap = await adminFirestore.collection("potd").doc(id).get();
  if (!docSnap.exists) {
  ...
  if (Array.isArray(body?.correct)) {
    updates.correct = body.correct.map((v: unknown) => String(v).toLowerCase()).filter(Boolean);
  }

  updates.updatedAt = admin.firestore.FieldValue.serverTimestamp();

  await adminFirestore.collection("potd").doc(id).set(updates, { merge: true });

  return NextResponse.json({ message: "POTD updated." });
}
...
export async function DELETE(
  ...
  await adminFirestore.collection("potd").doc(id).delete();
  return NextResponse.json({ message: "POTD deleted." });
}
