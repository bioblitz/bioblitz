...
function topicClass(topic: string): string {
  const map: Record<string, string> = {
    "Anatomy & Physiology": "cat-orange",
    "Cell Biology": "cat-teal",
    "Plant Biology": "cat-teal",
    "Genetics & Evolution": "cat-purple",
    Biosystematics: "cat-purple",
    Ecology: "cat-teal",
    Ethology: "cat-orange",
    Multiple: "cat-yellow",
  };
  return map[topic] || "cat-yellow";
}
...
export function generateNewsletterPageHtml(data: NewsletterData): string {
  ...
  const blitzSection = data.blitzOfWeek
    ? `
  <div class="section">
  ...
  <div class="section">
    ...
    <a href="${SITE}/potd" class="potd-link">
      <div class="potd-card">
        <div>
          <div class="potd-number">POTD · ${esc(data.potd.date)}</div>
          <div class="potd-title">${esc(data.potd.title)}</div>
          <div class="potd-desc">${esc(data.potd.topic)} · ${data.potd.multiSelect ? "Multi Select" : "Single Choice"}</div>
        </div>
        <span class="btn btn-outline">Solve it →</span>
      </div>
    </a>
  </div>
  <div class="divider"></div>`
  ...
  const eloRows = data.eloLeaderboard
    .map(
      (u, i) => `
    <div class="lb-row">
  ...
  <div class="section">
    ...
    <div class="empty-state">
      ...
      <div class="empty-body">Challenges are a great way to test your knowledge against friends. Pick a blitz, challenge someone, and see who comes out on top.</div>
    ...
    <a href="${SITE}/home/${s.id}" class="set-card-link">
      <div class="set-card">
        ...
        <div class="set-meta">
          ...
          ${s.averageRating > 0 ? `<span>${s.averageRating.toFixed(1)}/5</span>` : ""}
        </div>
      </div>
    </a>`,
    )
  ...
  <div class="section">
    ...
    <div class="section-title" style="margin-bottom:16px;">New This Week</div>
  ...
  <div class="section">
    <div class="section-header">
      <span class="section-tag tag-teal">Study Tip</span>
    </div>
    <div class="tip-card">
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .newsletter { max-width: 640px; margin: 25px auto 0; padding: 0 20px 80px; }
  ...
  .logo-wordmark { font-family: 'Outfit', sans-serif; font-size: 40px; font-weight: 800; color: var(--text); letter-spacing: 0.08em; }
...
.mito-link:hover {
  ...
  .blitz-meta { display: flex; gap: 16px; font-size: 13px; color: var(--text-muted); flex-wrap: wrap; }
  ...
  .set-meta { display: flex; gap: 10px; font-size: 11px; color: var(--text-dim); font-family: 'JetBrains Mono', monospace; }
  ...
  .footer { border-top: 1px solid var(--border); padding-top: 32px; text-align: center; }
  ...
  .footer-fine { font-size: 12px; color: var(--text-dim); line-height: 1.6; }
 ...
 <div class="footer">
  <div class="footer-fine">
    ...
    <br><a href="${SITE}/settings" class="mito-link">Manage Preferences</a><br>
      <br>&copy; ${new Date().getFullYear()} BioBlitz. Powered by <a href="https://mitosisphere.org" class="mito-link" target="_blank" rel="noopener noreferrer">
  Mitosisphere
</a>.<br>
